    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.schedules.update", [$schedule->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('doctor') ? 'is-invalid' : '' }}" name="doctor_id" id="doctor_id">
                    @foreach($doctors as $id => $doctor)
                        <option value="{{ $id }}" {{ (old('doctor_id') ? old('doctor_id') : $schedule->doctor->id ?? '') == $id ? 'selected' : '' }}>{{ $doctor }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('day') ? 'is-invalid' : '' }}" type="text" name="day" id="day" value="{{ old('day', $schedule->day) }}" required>
                @if($errors->has('day'))
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('start') ? 'is-invalid' : '' }}" type="text" name="start" id="start" value="{{ old('start', $schedule->start) }}">
                @if($errors->has('start'))
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('end') ? 'is-invalid' : '' }}" type="text" name="end" id="end" value="{{ old('end', $schedule->end) }}">
                @if($errors->has('end'))
